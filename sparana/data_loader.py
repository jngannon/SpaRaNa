...
class split_loader:
    ...
    '''This splits the data for my experiments with different classes, to test things like transfer learning and catastrophic forgetting. Remove classes inputs a list of classes to be removed and saved for later, add_classes gives a list of how many columns to add to the labels. For example. I want to remove 2 classes from the main dataset, and add one class back to try and train on both the removed classes with a different subset of parameters.'''
    
    def __init__(self, training_data, training_labels, test_data, test_labels, remove_classes, add_classes):
        ''' I will need to feed in extracted data, data might be stored in different formats.
        This is for shuffling and tracking training.'''
        self._training_data = training_data
        self._training_labels = training_labels
        ...
        # Delete removed classes based on the indices found abouve
    ...
    def removed_random_minibatch(self, batch_size, index, label_index):
    ...
    def removed_minibatch(self, batch_size, index, label_index = -1):
        ...
        if type(index) == list:
            ...
            for i in zip(index, label_index):
                ...
                these_labels = np.zeros((int(batch_size/len(index)), self._training_labels.shape[1]+self._add_classes))
                these_labels[:, i[1]] = 1
            ...
            labels = np.zeros((batch_size, self._training_labels.shape[1]+self._add_classes))
            labels[:, label_index] = 1
    ...
    def removed_test_set(self, index, label_index):
        if type(index) == list:
            # do the thing
            data = None
            labels = None
            for i in zip(index, label_index):
                ...
                these_labels = np.zeros((this_data.shape[0], self._training_labels.shape[1]+self._add_classes))
                these_labels[:, i[1]] = 1
                if type(data) != np.ndarray:
                    data = this_data
                    labels = these_labels
            ...
            labels = np.zeros((data.shape[0], self._training_labels.shape[1]+self._add_classes))
            labels[:, label_index] = 1
        return data, labels

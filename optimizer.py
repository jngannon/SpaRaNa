...
class madadad_optimizer:
    ...
    def train_step(self, inputs, labels):
        ...
        for i in range(len(grads)):
            ...
            self._adadad_stats[i] = self._friction*self._adadad_stats[i] + signs
        ...
        for i in range(len(grads)):
            if self._model._layer_type == 'Full':
                if self._l2_constant and not self._l1_constant:
                    self._model.layers[i]._weights += self._learning_rate*(grads[i][0] + self._adadad_stats[i]) - self._l2_constant/inputs.shape[0]*self._learning_rate*self._model.layers[i]._weights
                
                if self._l2_constant and self._l1_constant:
                    self._model.layers[i]._weights += self._learning_rate*(grads[i][0] + self._adadad_stats[i]) - self._l2_constant/inputs.shape[0]*self._learning_rate*self._model.layers[i]._weights - self._l1_constant/inputs.shape[0]*self._learning_rate*np.sign(self._model.layers[i]._weights)
...
class adadad_optimizer:
    ...
    def __init__(self, model, learning_rate, friction, momentum, l1_constant = None, l2_constant = None):
    ...
    def train_step(self, inputs, labels):
        ...
        for i in range(len(grads)):
            signs = np.sign(grads[i][0])
            self._adadad_stats[i] = (np.sign(self._adadad_stats[i][0]) == signs)*self._adadad_stats[i][0]
            self._adadad_stats[i] = self._adadad_stats[i]*self._friction + grads[i][0]
        ...
        for i in range(len(grads)):
            if self._model._layer_type == 'Full':
                if self._l2_constant and not self._l1_constant:
                    self._model.layers[i]._weights += self._learning_rate*(grads[i][0] + self._adadad_stats[i]*self._momentum) - self._l2_constant/inputs.shape[0]*self._learning_rate*self._model.layers[i]._weights
                
                if self._l2_constant and self._l1_constant:
                    self._model.layers[i]._weights += self._learning_rate*(grads[i][0] + self._adadad_stats[i]*self._momentum) - self._l2_constant/inputs.shape[0]*self._learning_rate*self._model.layers[i]._weights - self._l1_constant/inputs.shape[0]*self._learning_rate*np.sign(self._model.layers[i]._weights)
                
                if self._l1_constant and not self._l1_constant:
                    self._model.layers[i]._weights += self._learning_rate*(grads[i][0] + self._adadad_stats[i]*self._momentum) - self._l1_constant/inputs.shape[0]*self._learning_rate*np.sign(self._model.layers[i]._weights)
                
                if not self._l1_constant and not self._l2_constant:
                    self._model.layers[i]._weights += self._learning_rate*(grads[i][0] + self._adadad_stats[i]*self._momentum)
                    self._model.layers[i]._biases += self._learning_rate*grads[i][1]               
